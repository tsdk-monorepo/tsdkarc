 ...
 * internal memoization key and the SWR cache key, so path formatting never
 * has to be changed in two places.
 * @param path  string[]
 */
...
function resolveNodeOrThrow(client: any, path: string[]) {
  ...
  if (!node) {
    throw new Error(
      `[createSwrClient] No route found at path "${pathKey(path)}". ` +
...
function requireHandler(routeProxy: any, candidates: string[], path: string[]) {
  ...
  throw new Error(
    `[createSwrClient] Route at "${pathKey(
      path
...
type StaticQueryHook<F> = IsOptional<InferInput<F>> extends true
  ? (
      ...
      opts?: SWRConfiguration<InferOutput<F>> & { enabled?: boolean }
...
type LegacyQueryHook<T> = IsOptional<InferLegacyInput<T>> extends true
  ? (
      ...
      opts?: SWRConfiguration<InferLegacyOutput<T>> & { enabled?: boolean }
...
/** Builds a memoized useSWR-backed query hook bound to a specific route's fetch function and cache key. */
function makeQueryHook(fn: (...args: any[]) => any, key: string) {
  return function (
    data?: unknown,
    opts?: SWRConfiguration & { enabled?: boolean }
  ) {
    const isEnabled = opts?.enabled !== false;
    return useSWR(isEnabled ? [key, data] : null, () => fn(data), opts);
  };
}

/** Builds a memoized useSWRMutation-backed hook; `arg` from the trigger call is passed straight to `fn`. */
function makeMutationHook(fn: (...args: any[]) => any, key: string) {
...
function makeStreamHook(fn: (...args: any[]) => any) {
  return function (input?: unknown, opts?: { enabled?: boolean }) {
     ...
     * rerun the effect — and refetch the stream — every render regardless of
    ...
    {},
    {
      get(_, prop: string | symbol) {
        ...
        if (key === "useQuery") {
          ...
          result = makeMutationHook(handler, pathKey(path));
