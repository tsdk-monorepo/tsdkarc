...
async function run() {
  ...
  try {
    ...
    const updatedTheme = await client.v1.users.settings.updateTheme.mutation({
      theme: "light_mode",
    });
    ...
    const deviceRes = await client.v1.users.registerDevice.mutation({
  ...
  try {
    ...
    // 故意传入低于限制的值 (min: 8)
    await client.v1.users.updatePassword.mutation({ newPwd: "123" });
  } catch (err) {
    if (isRpcError(err)) {
