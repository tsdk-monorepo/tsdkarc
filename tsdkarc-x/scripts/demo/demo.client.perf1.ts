...
client2.v1.users.getProfile.query;
client2.v1.users.getProfile2.query;


// const swrHooks = createSwrClient<AppRoutes>(client);

const swrHooks2 = createSwrClient<AppRoutesSwr>(client2);
...
async function run() {
  ...
  try {
    ...
    const health = await client2.v1.users.health.query();
    ...
    const profile = await client2.v1.users.getProfile.query({
      includeHistory: true,
    });
    ...
    const uploadRes = await client2.v1.users.uploadAvatar.upload({
      file: dummyFile,
    ...
    const stream = await client2.v1.users.downloadLogs.stream({ lines: 3 });
    // 完美的流式迭代体验
    ...
    await client2.v1.users.triggerError.query();
  } catch (err) {
...

  {
    ...
    client2.mock.aiRoutes2.complete;
    client2.v1.users.health.query;
    swrHooks2.v1.users.downloadLogs.useStream({ lines: 100 });
    client2.v1.users;
    swrHooks2.mock.analyticsRoutes5.funnel.useMutation();
  }
}
