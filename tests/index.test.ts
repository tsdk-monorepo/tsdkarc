...
const makeQueue = (deps: readonly [ReturnType<typeof makeDb>, ReturnType<typeof makeCache>]) =>
...
describe('shutdown order', () => {
  it('shuts down in reverse boot order', async () => {
    ...
    const { stop } = await start([A], { log: noopLog });
    await stop();
...
describe('context', () => {
  it('module can write and downstream module can read', async () => {
    ...
    const app = await start([App], { log: noopLog });
...
describe('error handling', () => {
  it('throws and rolls back already-booted modules on failure', async () => {
    ...
    await expect(start([A], { log: noopLog })).rejects.toThrow('A exploded');
    expect(order).toContain('shutdown:B');
    expect(order).not.toContain('shutdown:A');
  });

  it('throws on circular dependency', async () => {
    const A = { name: 'A', modules: [] } as unknown as ReturnType<ReturnType<typeof defineModule>>;
    const B = { name: 'B', modules: [A] } as unknown as ReturnType<ReturnType<typeof defineModule>>;
    (A as { modules: unknown[] }).modules.push(B);
    await expect(start([A], { log: noopLog })).rejects.toThrow('Circular dependency');
  });
  ...
  it('throws on name collision — same name, different instance', async () => {
    ...
    await expect(start([Root], { log: noopLog })).rejects.toThrow('collision');
  });
  ...
  it('does not throw when same instance is referenced multiple times', async () => {
    ...
    await expect(start([A], { log: noopLog })).resolves.toBeDefined();
  });
...
describe('logging', () => {
  it('logs booting with description and booted', async () => {
    const log = vi.fn();
    const A = defineModule()({ name: 'A', description: 'test module' });
    await start([A], { log });
    expect(log).toHaveBeenCalledWith('info', 'A', 'booting', { description: 'test module' });
    expect(log).toHaveBeenCalledWith('info', 'A', 'booted');
  });

  it('logs boot_failed on error', async () => {
    const log = vi.fn();
    const A = defineModule()({
      name: 'A',
      boot: async () => {
    ...
    await expect(start([A], { log })).rejects.toThrow();
    expect(log).toHaveBeenCalledWith('error', 'A', 'boot_failed', 'oops');
  });

  it('logs shutdown', async () => {
    const log = vi.fn();
    const A = defineModule()({ name: 'A' });
    const { stop } = await start([A], { log });
    await stop();
    expect(log).toHaveBeenCalledWith('info', 'A', 'shutdown');
  });
});
